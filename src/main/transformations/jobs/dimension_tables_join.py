...
def dimesions_table_join(final_df_to_process,
    ...
    s3_customer_store_sales_df_join = s3_customer_store_df_join.join(sales_team_table_df.alias("st"),
                             ...
                             "inner")\
                .withColumnRenamed(existing="sales_person_first_name",new="st.first_name")\
                .withColumnRenamed(existing="sales_person_last_name",new="st.last_name")\
                .withColumnRenamed(existing= "sales_person_address",new="st.address")\
                .withColumnRenamed(existing="sales_person_pincode",new="st.pincode")\
